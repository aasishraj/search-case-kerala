...
from app.models.schemas import (
    ...
    CasePDFRequest,
    CasePDFResponse,
    CaseJudgementPDFRequest,
    CaseJudgementPDFResponse,
)
...
class KeralaCourtsService:
    ...
    def search_case(self, request: CaseSearchRequest) -> CaseSearchResponse:
        ...
        try:
            ...
            return CaseSearchResponse(cino=cino, case_no=case_no, success=True)
    ...
    def get_case_details(self, request: CaseDetailsRequest) -> CaseDetailsResponse:
        ...
        try:
            response = self.session.post(
                self.THIRD_URI,
                data={"cino": request.cino, "case_no": request.case_no},
                headers={
                    "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
                    "X-Requested-With": "XMLHttpRequest",
                    "Referer": self.FIRST_URI,
    ...
    def get_case_proceedings(
        self, request: CaseProceedingsRequest
    ) -> CaseProceedingsResponse:
        ...
        try:
            ...
            return CaseProceedingsResponse(
                cino=request.cino, proceedings_html=response.text
            )
    ...
    def get_case_pdf(self, request: CasePDFRequest) -> CasePDFResponse:
        ...
        try:
            ...
            response = self.session.get(
                pdf_uri, headers={"Referer": self.FIRST_URI}, timeout=30
            )
            response.raise_for_status()
    ...
    def get_case_file(self, request: CaseFileRequest) -> CaseFileResponse:
        ...
        try:
            fileview_uri = (
                f"{self.SIXTH_URI}?token={request.token}&lookups={request.lookups}"
            )
            response = self.session.get(
                fileview_uri, headers={"Referer": self.FIRST_URI}, timeout=30
            )
            response.raise_for_status()
    ...
    def download_interim_pdf(self, pdf_url: str) -> bytes:
        ...
        try:
            response = self.session.get(
                pdf_url, headers={"Referer": self.FIRST_URI}, timeout=30
            )
            response.raise_for_status()
    ...
    def get_case_judgement_pdf(
        self, request: CaseJudgementPDFRequest
    ) -> CaseJudgementPDFResponse:
        ...
        try:
            ...
            return CaseJudgementPDFResponse(pdf_url=pdf_url)
